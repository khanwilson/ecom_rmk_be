...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Storefront Service API')
    .setDescription('Storefront (KOL Affiliate) Service API Documentation')
    .setVersion('1.0')
    .addBearerAuth(
      {
        type: 'http',
    ...
    .addTag('storefront', 'Storefront management endpoints')
    .build();
  ...
  const port = parseInt(configService.get<string>('STOREFRONT_PORT', '3005'), 10) || 3005;
  await app.listen(port);
  console.log(`✅ Storefront service running on: http://localhost:${port}`);
